...
// BiChevronDown,BiChevronLeft,BiChevronRight,FiClock,BiDotsHorizontal
import { Menu, Transition } from '@headlessui/react'
import { BiChevronDown, BiChevronLeft, BiChevronRight, BiDotsHorizontal } from 'react-icons/bi'
...
const Calendar = () => {
  ...
  const [showEventForm, setShowEventForm] = useState(false); // State to control showing the event form
  ...
  const handleSubmitEvent = (e) => {
    ...
    if (eventForm.id === null) {
      const updatedSelectedDay = {
        ...selectedDay,
        events: [...selectedDay.events, eventForm],
      };
  ...
  const handleDeleteEvent = (eventId) => {
    const updatedEvents = selectedDay.events.filter((event) => event.id !== eventId);
    setSelectedDay({ ...selectedDay, events: updatedEvents });
  };

  return (
    <div className="mx-1 md:m-10 mt-24  md:p-10 bg-white rounded-3xl">
    <Header category="Page" title="Customers" />
    <div className="lg:flex lg:h-2/3 lg:flex-col md:mx-auto md:container">
        ...
        <div className="flex items-center">
          ...
          <div className="hidden md:ml-4 md:flex md:items-center">
            <Menu as="div" className="relative">
              ...
              >
                Month view
                <BiChevronDown className="ml-2 h-5 w-5 text-gray-400" aria-hidden="true" />
              </Menu.Button>
            ...
            <ol className="divide-y divide-gray-100 overflow-hidden rounded-lg bg-white text-sm shadow ring-1 ring-black ring-opacity-5">
              {selectedDay.events.map((event) => (
                <li key={event.id} className="group flex p-4 pr-6 focus-within:bg-gray-50 hover:bg-gray-50">
                  ...
                  <button
                    className="ml-2 flex-none self-center rounded-md border border-red-400 bg-white py-2 px-3 font-semibold text-red-400 shadow-sm hover:bg-red-50"
      ...
      {showEventForm && (
        <div className="py-6 px-4 sm:px-6 lg:hidden">
          <form onSubmit={handleSubmitEvent}>
            ...
            {/* Add more input fields for time, datetime, and href */}
            <div className="flex justify-end">
              <button
                type="submit"
                className="bg-indigo-600 py-2 px-4 text-white rounded-md hover:bg-indigo-700"
              >
                Save
              </button>
              <button
                type="button"
                className="ml-2 py-2 px-4 text-gray-600 rounded-md hover:text-gray-700"
